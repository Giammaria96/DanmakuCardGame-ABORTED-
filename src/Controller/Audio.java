...
public class Audio {
    ...
    Media BGM, SFX;
    MediaPlayer BGMPlayer, SFXPlayer;
    Double BGMVol, SFXVol;
    String Path;
    ...
    public void playBGM(String BGMname){
        if (BGM!=null){BGMPlayer.stop();}
        try {Path = (new File("src/Audio/BGM/"+BGMname+".mp3")).getCanonicalPath();
    ...
    public void playBGM(){
        if (BGM!=null){BGMPlayer.stop();}
        try {Path = (new File("src/Audio/BGM/MT"+randomNum+".mp3")).getCanonicalPath();
            Path = Path.replace(" ","%20");
            Path = Path.replace("\\","/");
            BGM = new Media("file:///"+Path);
            BGMPlayer = new MediaPlayer(BGM);
    ...
    public void playSFX(String SFXname){
        try {Path = (new File("src/Audio/SFX/"+SFXname+".wav")).getCanonicalPath();
            Path = Path.replace(" ","%20");
            Path = Path.replace("\\","/");
    ...
    public void testSFXVol(Double V){
        try {Path = (new File("src/Audio/SFX/se_changeItem.wav")).getCanonicalPath();
            Path = Path.replace(" ","%20");
            Path = Path.replace("\\","/");
            SFX = new Media("file:///"+Path);
            SFXPlayer = new MediaPlayer(SFX);
